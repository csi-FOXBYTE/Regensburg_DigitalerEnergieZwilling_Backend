...
function generateDependencyBom() {
  ...
  const rootRef = npmPurl(listed.name, listed.version);
  ...
  function visit(entries) {
    for (const [declaredName, node] of entries) {
      const name = node.from ?? declaredName;
      const ref = npmPurl(name, node.version);
      if (!components.has(ref)) {
        let metadata = {};
        try {
          metadata = JSON.parse(
            readFileSync(resolve(node.path, "package.json"), "utf8"),
          );
        } catch {
          // pnpm can report an optional platform package that is not materialized
          // locally. Its resolved name and version still belong in the graph.
        }
      ...
      const childRefs = childEntries.map(([childName, child]) =>
        npmPurl(child.from ?? childName, child.version),
      );
      const dependencies = dependencyMap.get(ref) ?? new Set();
      for (const childRef of childRefs) dependencies.add(childRef);
      dependencyMap.set(ref, dependencies);
      const nodeIdentity = node.path ?? ref;
      if (!expandedNodes.has(nodeIdentity)) {
        expandedNodes.add(nodeIdentity);
        visit(childEntries);
      }
    }
  }

  const rootEntries = dependencyEntries(listed);
  visit(rootEntries);
  const rootComponent = {
    type: "application",
  ...
  const dependencies = [
    {
      ref: rootRef,
      dependsOn: rootEntries.map(([name, node]) =>
        npmPurl(node.from ?? name, node.version),
      ).sort(),
    },
    ...[...components].map(([ref]) => ({
      ref,
      dependsOn: [...(dependencyMap.get(ref) ?? [])].sort(),
    })),
  ];
...
function applyReleaseVersion(bom) {
  ...
  const newRef = npmPurl(root.name, version);
